...
namespace Gameplay.Entities.Background
{
    public class EnvironmentController : MonoBehaviour
    {
        ...
        public void Init(Transform characterTransform)
        {
            ...
            _endPool = new ObjectPool(_endPrefab.gameObject, 0, transform);
        ...
        public float GetDistanceToFinishLine()
        {
            Vector2Int finishCell = new Vector2Int(0, _totalRows - 1);
            var position = CellToWorld(finishCell);
            var distance = position.y - _character.position.y;
            return distance;
        }
        ...
        private void ShiftColumns(int dx)
        {
            ...
            {
                (TileControl t, TileType tp, Vector2Int p) tmp = (_tiles[from, ay], _types[from, ay],
                    _gridPos[from, ay]);
                _tiles[from, ay] = _tiles[1, ay];
                _types[from, ay] = _types[1, ay];
                _gridPos[from, ay] = _gridPos[1, ay];
                _tiles[1, ay] = _tiles[to, ay];
                _types[1, ay] = _types[to, ay];
                _gridPos[1, ay] = _gridPos[to, ay];
                _tiles[to, ay] = tmp.t;
                _types[to, ay] = tmp.tp;
                _gridPos[to, ay] = tmp.p;
            }
        }
        ...
        private void ShiftRows(int dy)
        {
            ...
            {
                (TileControl t, TileType tp, Vector2Int p) tmp = (_tiles[ax, from], _types[ax, from],
                    _gridPos[ax, from]);
                _tiles[ax, from] = _tiles[ax, 1];
                _types[ax, from] = _types[ax, 1];
                _gridPos[ax, from] = _gridPos[ax, 1];
                _tiles[ax, 1] = _tiles[ax, to];
                _types[ax, 1] = _types[ax, to];
                _gridPos[ax, 1] = _gridPos[ax, to];
                _tiles[ax, to] = tmp.t;
                _types[ax, to] = tmp.tp;
                _gridPos[ax, to] = tmp.p;
            }
        }
